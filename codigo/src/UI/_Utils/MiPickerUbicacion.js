...
export default class MiPickerUbicacion extends React.Component {
  ...
  componentDidMount() {
    Animated.spring(this.animInicio, {
      toValue: 1,
      delay: 300,
      duration: 300,
  ...
  onContenedorBusquedaPress = () => {
    ...
    Animated.timing(this.animSugerencias, {
      toValue: 1,
      duration: 300
    }).start();
  };
  ...
  onBtnVolverPress = () => {
    if (this.state.mostrarPanelBusqueda == true) {
      this.onBtnCancelarBusquedaPress();
      return;
    }
  ...
  mostrarDialogoObservaciones = () => {
    ...
    App.navegar("UbicacionDescripcion", {
      callback: this.onDescripcion
    });
    ...
    return (
      ...
      >
        <Card style={[styles.contenedorBusqueda]}>
          ...
          <Animated.View
            pointerEvents={
              this.state.mostrarPanelBusqueda == true ? "auto" : "none"
            }
            style={[
          ...
          >
            {this.state.cargando == false && (
              ...
              >
                <Icon name="magnify" style={styles.btnCancelarBusquedaIcono} />
              </Button>
            )}
          </Animated.View>
          ...
          {this.state.cargando == true ? (
            <View
              style={{ position: "absolute", right: 12, top: -14, zIndex: 20 }}
            >
              <Spinner color="green" />
            </View>
          ) : (
  ...
  renderMapa() {
    if (
      this.state.posicionInicial == undefined ||
      this.state.cargarMapa != true
    )
      return null;
        ...
        >
          {this.state.data != undefined && (
            <Marker
              ...
              coordinate={{
                latitude:
                  parseFloat(this.state.data.latitud.replace(",", ".")) || 0,
                longitude:
                  parseFloat(this.state.data.longitud.replace(",", ".")) || 0
              }}
  ...
  renderCard() {
    ...
    if (
      this.state.data != undefined &&
      this.state.data.direccion != undefined &&
      this.state.data.direccion != ""
    ) {
      direccion =
        (this.state.data.sugerido == true ? "Aprox. en " : "") +
        toTitleCase(this.state.data.direccion.trim());
    }
    ...
    if (this.state.data != undefined && this.state.data.cpc != undefined) {
      cpcNombre =
        "Nº " +
        this.state.data.cpc.numero +
        " - " +
        toTitleCase(this.state.data.cpc.nombre).trim();
    }
  ...
  renderSugerenciaItem = item => {
    return (
      <MiPickerUbicacionItem
        onPress={this.onSugerenciaClick}
        data={item.item}
      />
    );
  };
        ...
        >
          ...
          >
            {this.state.sugerencias != undefined &&
              this.state.sugerencias.length == 0 && <Text>Sin resultado</Text>}
      ...
      >
        <View style={{ display: "flex", width: "100%" }}>
          ...
          <Text style={{ alignSelf: "flex-start" }}>
            {toTitleCase(item.direccion).trim()}
          </Text>
        </View>
      </ListItem>
    );
