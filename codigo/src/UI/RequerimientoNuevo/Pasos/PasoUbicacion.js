...
export default class RequerimientoNuevo_PasoUbicacion extends React.Component {
  ...
  renderViewSeleccionado() {
    ...
    if (this.state.ubicacion.direccion != undefined) {
      textoDireccion =
        (this.state.ubicacion.sugerido == true ? "Aproximadamente en " : "") +
        this.state.ubicacion.direccion.trim();
    }
    ...
    if (this.state.ubicacion.cpc != undefined) {
      textoCpc =
        "Nº " +
        this.state.ubicacion.cpc.numero +
        " - " +
        toTitleCase(this.state.ubicacion.barrio.nombre);
    } else {
      textoCpc = "Sin datos";
    }
    let textoBarrio;
        ...
        <View>
          <View>
            <View style={{}}>
              <View style={{ padding: 16 }}>
                <MiItemDetalle
                  titulo={texto_Direccion}
                  subtitulo={textoDireccion}
                />
                <View style={{ height: 8 }} />
                <MiItemDetalle
                  titulo={texto_Observaciones}
                  subtitulo={textoObservaciones}
                />
                <View style={{ height: 8 }} />
