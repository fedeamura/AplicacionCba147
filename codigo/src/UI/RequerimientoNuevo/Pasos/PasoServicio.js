...
export default class RequerimientoNuevo_PasoServicio extends React.Component {
  ...
  cumpleBusquedaMotivoBusqueda = (item, texto) => {
    ...
    return (
      cumpleNombre == true || cumpleKeyword == true || cumpleServicio == true
    );
  };
    ...
    return (
      <View key={item.motivoId} style={{ width: "100%" }}>
        <Text style={{ flex: 1, alignSelf: "flex-start", fontSize: 20 }}>
          {toTitleCase(item.motivoNombre).trim()}
        </Text>
        <Text
          style={{
            flex: 1,
  ...
  onBotonSiguientePress = () => {
    if (
      ...
      this.state.descripcion.length < 20
    ) {
        ...
        <View style={{ width: wCirculo }} key={servicio.id}>
          <CardCirculo
            ...
            textoStyle={{
              ...
              maxWidth: wCirculo * 0.8,
              minWidth: wCirculo * 0.8
            }}
          />
        </View>
      );
  ...
  renderViewSeleccionarMotivo() {
    const nombreServicio =
      this.state.servicioNombre == undefined
        ? ""
        : toTitleCase(this.state.servicioNombre);
    ...
    return (
      <MiView visible={this.state.mostrarMotivo}>
        <View style={{ padding: 16 }}>
          ...
          >
            <MiItemDetalle
              titulo={texto_ServicioSeleccionado}
  ...
  renderViewMotivoSeleccionado() {
    ...
    const nombreServicio = toTitleCase(
      this.state.servicioNombre || "Sin datos"
    ).trim();
    const nombreMotivo = toTitleCase(
      this.state.motivoNombre || "Sin datos"
    ).trim();

    return (
      <MiView padding={false} visible={this.state.mostrarResultado}>
        <View style={{ minHeight: 420, display: "flex" }}>
          <View style={{ padding: 16, flex: 1 }}>
            ...
            <MiItemDetalle
              titulo={texto_MotivoSeleccionado}
              subtitulo={nombreMotivo}
            />
            <View style={{ height: 8 }} />
            ...
            {this.state.descripcion != undefined &&
              ...
              this.state.descripcion.length < 20 && (
                <Text>{`${this.state.descripcion.length}/20 caracteres`}</Text>
              )}
            <View style={{ height: 16 }} />
          </View>
...
const texto_Hint =
  "Indique de la forma más detallada posible toda la información asociada al requerimiento...";
